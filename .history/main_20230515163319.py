...
def add_expense():
    ...
    amount = float(entry_amount.get())  # Convert amount to float
    today = date.today()
    cursor.execute(
        """
        INSERT INTO expenses (name, amount, date) VALUES (?, ?, ?)
        """,
        (name, -amount, today)  # Subtract the amount from the balance
    )
    connection.commit()
...
def add_salary():
    ...
    salary = tk.simpledialog.askinteger("Add Salary", "Enter your salary:")
    if salary is None:  # If the user cancels the dialog
        return

    # Insert the salary as a new expense in the database
...
def reset_all():
    ...
    # Prompt the user to enter a new salary
    new_salary = tk.simpledialog.askinteger("New Salary", "Enter your new salary:")
    if new_salary is None:  # If the user cancels the dialog
        return

    # Update the balance with the new salary
    cursor.execute("INSERT INTO expenses (name, amount, date) VALUES (?, ?, ?)", ("Salary", new_salary, date.today()))
    connection.commit()
...
button_add_salary = tk.Button(window, text="Add Salary", command=add_salary)
button_add_salary.pack()
...
button_chart.pack()
button_reset = tk.Button(window, text="Reset All", command=reset_all)
button_reset.pack()
